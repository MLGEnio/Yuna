...
const getVisibleproducts = (
  ...
  return Products.filter((product) => {
    let typeMatch;
    if (type && product.type) typeMatch = product.type === type;
    else typeMatch = true;

    let brandMatch;
    if (brand && product.brand) brandMatch = brand.includes(product.brand);
    ...
    const startPriceMatch =
      typeof priceMin !== "number" || priceMin <= product.price;
    const endPriceMatch =
      typeof priceMax !== "number" || product.price <= priceMax;

    return (
      typeMatch && brandMatch && colorMatch && startPriceMatch && endPriceMatch
    );
